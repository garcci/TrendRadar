...
class GitHubIssuesMemory:
    ...
    def _format_issue_body(self, metadata: Dict) -> str:
        """格式化 Issue body，自动处理长度限制"""
        excerpt = metadata.get('excerpt', '')
        keywords = metadata.get('keywords', [])
        hot_topics = metadata.get('hot_topics', [])
        platforms = metadata.get('platforms', [])
        
        # 构建基础内容
        body = f"""## Article Metadata
...
"""
        ...
        # 检查长度限制
        if len(body) > self.MAX_ISSUE_BODY_LENGTH:
            logger.warning(f"Issue body too long ({len(body)} chars), truncating excerpt...")
            
            # 计算需要截断多少字符
            overflow = len(body) - self.MAX_ISSUE_BODY_LENGTH + 100  # 额外留100字符余量
            
            if len(excerpt) > overflow:
                # 截断excerpt
                truncated_excerpt = excerpt[:len(excerpt) - overflow] + "...\n\n[内容已截断 - 原始内容过长]"
...
"""
                ...
                # 如果excerpt本身不长，截断其他部分
                body = body[:self.MAX_ISSUE_BODY_LENGTH - 100] + "\n\n[内容已截断 - 原始内容过长]"
